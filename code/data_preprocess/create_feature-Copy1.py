...
def one_class_svm2(x,kernel,degree):
    stdsc = StandardScaler() 
    x = stdsc.fit_transform(x)
    ...
    clf = svm.OneClassSVM(nu=0.1, kernel=kernel, gamma='scale',verbose=True, random_state=random_seed,degree=2)
    clf.fit(x2)
    x3 = clf.score_samples(x)
    print(x3.shape)
    return x3
...
#     svm_rbf = pd.DataFrame(svm_rbf,columns=['svm_rbf'])
    ...
    print('poly2')
    svm_poly2 = one_class_svm2(all_data.drop(columns=delete_list),'poly',2)
    print('poly3')
    svm_poly3 = one_class_svm2(all_data.drop(columns=delete_list),'poly',3)
    print('poly4')
    svm_poly4 = one_class_svm2(all_data.drop(columns=delete_list),'poly',4)
    svm_poly2 = pd.DataFrame(svm_poly2,columns=['svm_poly2'])
    svm_poly3 = pd.DataFrame(svm_poly3,columns=['svm_poly3'])
    svm_poly4 = pd.DataFrame(svm_poly4,columns=['svm_poly4'])
    svm_poly = pd.concat([svm_poly2,svm_poly3,svm_poly4],axis=1)
...
#     svm_linear.to_csv('../../data/preprocess/svm_linear_feature.csv',index=False)
#     print('sigmoid')
#     svm_linear = one_class_svm(all_data.drop(columns=delete_list),'sigmoid')
#     svm_linear = pd.DataFrame(svm_linear,columns=['sigmoid'])
#     svm_linear.to_csv('../../data/preprocess/svm_sigmoid_feature.csv',index=False)
