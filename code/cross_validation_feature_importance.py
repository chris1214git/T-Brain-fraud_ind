...
def feature_importance_test(model,X_test1,y_test1,categorical_features_indices,importance_type):
    train_pool=Pool(X_test1, y_test1,cat_features=categorical_features_indices)
    feature_names = X_test1.columns
    delete_col = []

    if importance_type=='PredictionValuesChange':
        feature_importances = model.get_feature_importance(train_pool,type=importance_type)
        for score, name in sorted(zip(feature_importances, feature_names), reverse=True):
            if score<0.1:
        ...
        for score, name in sorted(zip(feature_importances, feature_names), reverse=True):
            if score<0.0:
                delete_col.append(name)    
    elif importance_type='Shapvalue':
        feature_importances = model.get_feature_importance(train_pool,type=importance_type)
...
def main():
    ...
    feature_important_types=['PredictionValuesChange','LossFunctionChange','Shapvalue']
    ...
    for fi_type in feature_important_types:
        ...
        for i in range(3):
            ...
            delete_col = feature_importance_test(model,X_test,y_test,categorical_features_indices)
        ...
        for i in range(3):
            ...
            X_test_pred[X_test_pred>th]=1
            X_test_pred[X_test_pred<=th]=0
            np.save('../prediction/validation/{}_X_train_{}_{}'.format(submit_file_name,fi_type,i+1),X_train_pred)
            np.save('../prediction/validation/{}_X_test_{}_{}'.format(submit_file_name,fi_type,i+1),X_test_pred)
        ...
        submit_file_name2 = submit_file_name+'_'+fi_type
