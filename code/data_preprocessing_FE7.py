...
for d in diff_list:
    print(d)
    for b in bacno_list:
        all_data.loc[all_data['bacno']==b,'bacno_{}_diff1'.format(d)]=all_data.loc[all_data['bacno']==b,d].diff(periods=1).fillna(value=-1)
        all_data.loc[all_data['bacno']==b,'bacno_{}_diff2'.format(d)]=all_data.loc[all_data['bacno']==b,d].diff(periods=2).fillna(value=-1)
        all_data.loc[all_data['bacno']==b,'bacno_{}_diff3'.format(d)]=all_data.loc[all_data['bacno']==b,d].diff(periods=3).fillna(value=-1)
